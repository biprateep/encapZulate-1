...
def CapsNet(input_shape, num_class, routings, dim_capsule, **kwargs):
    ...
    collect_layer_train = Concatenate()([masked_by_y, squash(val_input)])
    collect_layer_test = Concatenate()([masked, squash(val_input)])
    # collect_layer = Concatenate()([digits_caps_flat, m_capsnet.output])
    redshift_input = Input(shape=(dim_capsule + 6,))
    ...
    train_model = models.Model(
        ...
        [m_capsnet.output, decoder(masked_by_y), redshift(collect_layer_train)],
    )
    ...
    manipulate_model = models.Model(
        ...
        [masked, m_capsnet.output, decoder(masked), redshift(collect_layer_test)],
    )
    train_model.summary()
